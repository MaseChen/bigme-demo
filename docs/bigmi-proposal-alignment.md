...
- `Security Center` 展示敏感操作的二次确认、alarm posture 和运行留痕
...
- Python Runtime
  - [bigmi_runtime.py](../appdaemon/conf/apps/bigmi_runtime.py)
  - [house_program_demo.py](../appdaemon/conf/apps/house_program_demo.py)
- Blueprint 模板
  - [programmable_scene_trigger.yaml](../ha/blueprints/automation/bigmi/programmable_scene_trigger.yaml)
- Webhook / API
  - `POST /api/webhook/bigmi_demo_scene`
- Visual Rules
  - `Node-RED` flow editor on `http://localhost:1880`
...
这些状态在 [bigmi_demo.yaml](../ha/packages/bigmi_demo.yaml) 和 `Bigmi Console` 中都能看到，适合在 PPT 里先讲“统一控制面”，后续再替换成真实设备。
...
- Node-RED 可视化流程
- Trace 和日志
- Bigmi 的 `Home / Spaces / Marketplace / Security / Developer` 演示面板
